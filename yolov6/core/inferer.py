...
class Inferer:
    ...
    def infer(self, conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf):
        ...
        for img_path in tqdm(self.img_paths):
            ...
            # nms_list.append((time.time() - t1)) # 计算平均nms耗时
            ...
            # Save results (image without detections)
            if save_img:
                cv2.imwrite(save_path, img_ori)
            
            fps_list.append(1 / (time.time() - t1)) # 计算平均fps

        print('mean FPS:', sum(fps_list)/len(fps_list)) # 打印平均fps
        # print('mean NMS:', sum(nms_list)/len(nms_list)) # 打印平均nms

        # 统计预测框（漏检率误诊率用）
        Note=open('cal/preds.txt',mode='w')
